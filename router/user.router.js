...
router.use('/details', async (req, res, next)=>{
  try{
    ...
    if(!user){
      return res.status(400).json({success: false, errorMessage:`No user with id: ${userId} found`})
    }
    ...
    res.status(400).json({success: false, message:`No user with id: ${userId} found`, errorMessage: err.message})
  }
})
...
.post(async(req, res)=>{
  try{
    ...
    let search = await User.findOne({avatarname : userUpdate.avatarname})
